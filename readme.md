...
1. Set up your Craft CMS backend:
    ...
    ddev php ./backend/craft install
    ```
...
1. Generate a token for the _Posts_ GraphQL Schema:
    ...
    ```bash
    ddev php ./backend/craft graphql/list-schemas
    ddev php ./backend/craft graphql/create-token [schema-uid]
    ```

1. Configure your frontend environment:

    - Copy `frontend/.env.example` to `frontend/.env`;
    - Update `NUXT_GRAPHQL_TOKEN` to the token generated in the previous step;
...
Nuxt lives in the `frontend/` directory. All NPM commands should be prefixed `--prefix=frontend` that ensures tasks are run in the appropriate directory:
...
- Update `NUXT_BASE_URL` in `frontend/.env`;
...
> Check ./config/general.php > allowedGraphqlOrigins()
